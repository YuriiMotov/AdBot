...
class DataCached():
    ...
    def _user_to_dict(self, user: m.User) -> UserDict:
        return {
            ...
            'msgs_queue_len': len(user.forward_queue),
        }

    async def refresh_user_data(self, session: Session, user_id: int) -> UserDict:
        """
        ...
        user_data['msgs_queue_len'] = len(user.forward_queue)
        return user_data

    def _load_users_data(self, session: Session) -> None:
        if self._users_cache is None:
            st = select(m.User).where(m.User.menu_closed == False)
    ...
    async def add_user(self, session: Session, user_id: int, user_name: str):
        if self._users_cache is None:
            self._load_users_data(session)
        user = m.User()
        user.id = user_id
    ...
    async def forward_msgs(self, session: Session, forward_func: ForwardFunc) -> None:
        st = select(m.User).where(m.User.forwarding == True)
        users = session.scalars(st).all()
        for user in users:
